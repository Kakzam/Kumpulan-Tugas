...
public class ItemSosmed extends RecyclerView.Adapter<ItemSosmed.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.hapus.setOnClickListener(view -> FirebaseFirestore.getInstance().collection("go_creative").document("sosmed").collection("-").document(key.get(position)).delete().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Toast.makeText(context, "Kamu berhasil menghapus sosmed", Toast.LENGTH_LONG).show();
            } else
                Toast.makeText(context, "Kamu gagal menghapus sosmed", Toast.LENGTH_LONG).show();
        }));
